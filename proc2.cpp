...
bool sieve_of_erastosthenes(long long key, long long n)
{
...
bool check_key(long long key, long long n)
{
    ...
    if (!sieve_of_erastosthenes(key, n))
		return false;
...
int main()
{
    ...
    int recv_bytes{
        static_cast<int>(recv(p1_info.descriptor, buffer, buffer_size - 1, 0))};
    ...
    if (check_key(key, n)) {
        ...
        try {
            ...
            std::cerr << "Couldn't create a descriptor, exiting... "
                      << std::endl;
            std::cerr << error << std::endl;
            return 1;
        }
